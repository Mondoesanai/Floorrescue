...
export function Hero({ copy, posterSrc, quoteHref }: HeroProps) {
  return (
    <section className="relative flex min-h-[85svh] items-end overflow-hidden bg-charcoal-950">
      {/* The background image never fades — it's the same frame the
          cinematic journey froze on, so the transition into this page reads
          as one continuous held shot. Only the HTML content on top fades in,
          instead of the whole hero (background included) fading from black. */}
      ...
      <div className="animate-[hero-fade-in_700ms_cubic-bezier(0.22,1,0.36,1)] relative z-10 w-full px-6 pb-16 sm:px-10 sm:pb-24">
